...
public class RaspagerProtocol implements TransmitterDeviceProtocol {
	...
	// The RaspagerProtocol is a generic one and can also been used for XOS,
	// PR430, SDR and DV4mini Transmitter
	...
	public void handleWelcome(TransmitterDevice transmitterDevice, PrintWriter toServer, BufferedReader fromServer)
			throws TransmitterDeviceException, InterruptedException, IOException {
		...
		case DV4mini:
			...
			break;
		default:
			throw new TransmitterDeviceException(
					"UNSUPPOTED_DEVICE_TYPE: Initialize RaspagerProtocol with unsupported DeviceType");
		}
		...
		if (deviceType == Transmitter.DeviceType.XOS) {
			...
			if (!msg.contains("/")) {
				throw new TransmitterDeviceException(
						"WRONG SID: " + msg + " Expected SID to start with " + expectedSid);
			} else {
		...
		/*
		 * Might be useful later for version control Pattern sid = Pattern.
		 * compile("\\[(\\w+) v(\\d+[.]\\d+)-SCP-#(\\d+)\\][ ]?\\d?[ ]?\\d?[ ]?\\d?[ ]?\\d?[ ]?\\d?[ ]?"
		 * ); // [RasPager v1.0-SCP-#2345678] Matcher sid_matchm =
		 * sid.matcher(msg); if (!sid_matchm.matches()) { // throw new
		 * TransmitterDeviceException("WRONG SID: " + msg); } String name =
		 * sid_matchm.group(1); // group(0) is whole search string, no need to
		 * check correct name because ip already checked String version =
		 * sid_matchm.group(2); String supportedProtocols = sid_matchm.group(3);
		 * // maybe useful later if new features are introduced and some devices
		 * are not yet updated
		 */
	...
	public void handleMessage(Message message, PrintWriter toServer, BufferedReader fromServer)
			throws InterruptedException, TransmitterDeviceException, IOException {
