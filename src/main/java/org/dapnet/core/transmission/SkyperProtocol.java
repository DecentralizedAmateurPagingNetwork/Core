...
public class SkyperProtocol implements PagerProtocol {
	...
	public List<Message> createMessagesFromCall(Call call) {
		...
		for (int address : addresses) {
			messages.add(new Message(call.getText(), address,
	...
	public Message createMessageFromTime(Date date) {
		...
		// Create Message
		Message message = new Message(timeString, 2504, Message.MessagePriority.TIME, Message.FunctionalBits.NUMERIC);
	...
	public Message createMessageFromNews(News news) {
		...
		// Create Message
		Message message = new Message(newsString, 4520, Message.MessagePriority.NEWS, Message.FunctionalBits.ALPHANUM);
	...
	public Message createMessageFromActivation(Activation activation) {
		...
		// Create Message
		Message message = new Message(activationString, activation.getNumber(), Message.MessagePriority.ACTIVATION,
