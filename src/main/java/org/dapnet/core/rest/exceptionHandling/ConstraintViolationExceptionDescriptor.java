...
public class ConstraintViolationExceptionDescriptor extends ExceptionDescriptor {
	...
	public ConstraintViolationExceptionDescriptor(int code, String message, String description,
			Set<ConstraintViolation<?>> violations) {
		super(code, message, description);
		this.violations = new ArrayList<>();
		// lambda expressions seems not to be supported by this version of
		// jersey
	...
	public static class Violation implements Serializable {
		private static final long serialVersionUID = -8968015977445102294L;
		private int code;
		...
		public Violation(ConstraintViolation<?> violation) {
			...
			if (violation.getConstraintDescriptor().getAnnotation().annotationType().getSimpleName()
					.equals("ValidName")) {// Constraint Violation due to
											// invalid names
