...
public class GsonTypeAdapter implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(final Gson gson, final TypeToken<T> tokenType) {
        ...
        return new TypeAdapter<T>() {
            //Validation Workaround because IpAddress is already validated while conversation from JSON to object
            @Override
            public T read(JsonReader reader) throws IOException {
                ...
                if (returnValue instanceof IpAddress) {
                    ...
                    if (port > 65535 || port < 1) // Valid ports are between 1 and 65535
                        throw new InvalidAddressException();
                }
                return returnValue;
            }
